        ...
        <section class="Fsl">
            <ul>
                ...
                <c:forEach items="${productList}" var="productList" varStatus="status">
                        <li>
                            <a href="/toDetail?id=${productList.id}"  title="电影名"><img src="${ctx}${productList.imageUrl}"></a>
                            <p>电影名：${productList.productName}</p>
                            <hr>
                            <p>价格:${productList.price}</p>
                            <hr>
                            <p>购票量:${productList.buyCount}</p>
                            <p>
                                <span class="DSBUTTON" style="float: right"><a href="/toDetail?id=${productList.id}"  class="Fontfff">电影详情</a></span>
                            </p>
                        </li>
                </c:forEach>
            </ul>
            ...
            <aside>
                ...
                <c:forEach items="${HotProductList}" var="HotProductList" varStatus="status">
                    <div class="C-list">
                        <a href="/toDetail?id=${HotProductList.id}" title="电影名"><img src="${ctx}${HotProductList.imageUrl}" width="100%" height="30%"></a>
            ...
            <div class="TurnPage">
                ...
                <c:if test="${page.pageNum > 1}"> <a href="findProductByCategory?page=${pages.prePage}"><span class="PNumber">上一页</span></a></c:if>
                <c:if test="${page.pageNum != page.navigateLastPage}"><a href="findProductByCategory?page=${pages.nextPage}"><span class="PNumber">下一页</span></a></c:if>
